...
function SelectTrigger({ className, size = "default", children, ...props }) {
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        // defaults
        "flex h-[48px] w-full items-center justify-between rounded-md border border-white/10 bg-primary px-4 py-2 text-base text-white/60 placeholder:text-white/60 outline-none focus:border-accent",
        className
      )}
      {...props}
    >
...
function SelectItem({ className, children, ...props }) {
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        // use highlighted state (not focus) + fix text contrast
        "relative flex w-full select-none items-center gap-2 rounded-sm py-1.5 pl-2 pr-8 text-sm outline-none cursor-default",
